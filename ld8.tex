...
Suppose $u$ and $v$ are percentile ranks of continuous random variables $x$ and $y$.  Then $(u,v)$ has standard uniform marginals and joint distribution $C$, a copula \citep{nelson1999ic}. Of interest is the dependence structure underlying $(x,y)$.

Spearman's $\rho$ \citep{embrechts2002correlation} between $x$ and $y$  is a common measure of overall rank dependence, the dependence between percentile ranks $u$ and $v$. Rank dependence is not distorted by marginal distributions unlike for example Pearson's correlation \citep{mcneil2005qrm} which is generally not $\pm 1$ when $x$ and $y$ are perfectly dependent unless $x$ and $y$ are linear in one another. Spearman's $\rho$ can also be applied to estimate copula parameters using the method of moments (\cite{kojadinovic2010comparison}, \cite{bouye2000copulas}). However a drawback of Spearman's $\rho$ is that it does not capture local dependence which may vary across the joint distribution as explained in section \aref{sreview}. This drawback is addressed by a generalisation of Spearman's $\rho$ discussed in the following.

Spearman's $\rho$ is defined and can be rewritten as
\begin{equation}\label{spearman}
\rho \equiv \cor(v,u) = \frac{\cov(v,u)}{\cov(u,u)}
=\frac{\int_0^1 \cov\{v,(u>\alpha)\} \de \alpha }{\int_0^1 \cov\{u,(u>\alpha)\} \de \alpha }
\end{equation}
where $\cov$ and $\cor$ represent covariance and correlation, respectively. In addition $(u>\alpha)$ is a random variable equal to 1 if $u>\alpha$ and 0 otherwise. The final expression in \eref{spearman} relies on the ``layer decomposition" of $u$
\begin{equation}\label{decompose}
u= \int_0^1 (u>\alpha)\de\alpha\ .
\end{equation}
The left hand side of \eref{decompose} is $\int_0^u 1\de \alpha=u$. \eref{decompose} decomposes $u$ into layers $(u>\alpha)\de\alpha$ across $0\le\alpha\le 1$. The term layer is used since integrating $(u>\alpha)\de\alpha$ over a subset $[a,b]$ of the unit interval yields $\min\{\max(u-a,0),b-a\}$ which corresponds to layers covered by excess-of-loss reinsurance \citep{wang1995insurance} and debt tranches \citep{mandel2012role}. The infinitesimally small layer $(u>\alpha)\de\alpha$ captures movements in $u$ at $\alpha$ since it jumps when $u$ crosses $\alpha$ and is constant over other values of $u$.

Removing the integrals in the final expression of \eref{spearman} yields the $\alpha$--layer dependence  between $v$ and $u$:
...
\end{equation}
Denominators in \eref{definition} are independent of $C$ and imply $\ell_\alpha=1$ if $u=v$ and $\ell_\alpha=-1$ if $u=1-v$.  Further $-1\leq \ell_\alpha \leq 1$. Independence implies $\ell_\alpha=0$. These and other properties of $\ell_\alpha$ are explored in sections \aref{scoherence} and \aref{sproperties}. Since $(u>\alpha)$ captures movements in $u$ at $\alpha$, $\ell_\alpha$ generalises Spearman's $\rho$ by measuring local dependence between $v$ and $u$ at $\alpha$. This interpretation is further discussed below and in subsequent sections.

The local dependence interpretation of $\ell_\alpha$ can be alternatively shown by expanding the covariances  in \eref{definition} and manipulating to yield
...
Rewriting Spearman's $\rho$ in \eref{spearman} in terms of $\ell_\alpha$ yields
\begin{equation}\label{wtdaverage}
\rho = \frac{\int_0^1 \cov\{u,(u>\alpha)\} \ell_\alpha \de \alpha }{\int_0^1 \cov\{u,(u>\alpha)\} \de \alpha }
= \int_0^1 w_\alpha \ell_\alpha \de \alpha
\cq w_\alpha\equiv 6\alpha(1-\alpha) \; .
\end{equation}
Hence Spearman's $\rho$ is a weighted average of $\ell_\alpha$ across $0\le\alpha\le 1$. The weights are $w_\alpha$ and integrate to $1$. Therefore whilst Spearman's $\rho$ measures overall dependence between $u$ and $v$, $\ell_\alpha$ performs the same but locally and averaging values of $\ell_\alpha$ yields $\rho$. Further note $w_\alpha$ has minimum $0$ at $\alpha=0$ and $1$, and increases symmetrically to maximum at $\alpha=0.5$. This implies Spearman's $\rho$ places less emphasis on tail dependence which may be undesirable in finance or insurance where tail dependence is present. Modifying the weights leads to alternate measures of overall dependence. This is further explored in section \aref{saltoverall}.
...
If $(u,v)$ is exchangeable then layer dependence $\ell_\alpha$ measures the lack of discordance and dispersion at $\alpha$:
...
\begin{figure}
  \begin{center}
    \begin{tabular}{ccc}
      \resizebox{40mm}{!}{\includegraphics{normal1.eps}}
            \resizebox{40mm}{!}{\includegraphics{student1.eps}}
      \resizebox{40mm}{!}{\includegraphics{structural21.eps}} \\
      \resizebox{40mm}{!}{\includegraphics{gumbel1.eps}}
            \resizebox{40mm}{!}{\includegraphics{structural11.eps}}
      \resizebox{40mm}{!}{\includegraphics{clayton1.eps}} \\
            \resizebox{40mm}{!}{\includegraphics{frank1.eps}}
            \resizebox{40mm}{!}{\includegraphics{structural31.eps}}
      \resizebox{40mm}{!}{\includegraphics{structural41.eps}} \\
    \end{tabular}
